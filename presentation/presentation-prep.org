...
* Intro - Raw Plot of CW001:
#+begin_src R :results file graphics :file img/cw001_raw.png
tuneR::readWave("../data/cw001.wav") %>%
  tuneR::plot(main = "Raw Plot of `cw001`")
#+end_src

* Intro - Normalized Plot of CW001:
#+begin_src R :results file graphics :file img/cw001_norm.png
library(tidyverse)
tuneR::readWave("../data/cw001.wav") %>%
  tuneR::normalize() %>%
  plot(main = "normalized waveform of `cw001`")

#+end_src

* Intro - Clean, Normalized Plot of CW009:
#+begin_src R :results file graphics :file img/cw009_norm.png
library(tidyverse)
tuneR::readWave("../data/cw009.wav") %>%
  wav_remove_NAs() %>%
  tuneR::normalize() -> cw009
  plot(cw009, main = "normalized waveform of `cw009`")

#+end_src

* Analysis - Normalized Plot of CW047:
#+begin_src R :results file graphics :file img/cw047_norm.png
library(tidyverse)
tuneR::readWave("../data/cw047.wav") %>%
  wav_remove_NAs() %>%
  tuneR::normalize() -> cw047
  plot(cw047, main = "normalized waveform of `cw047`")
...
#+RESULTS: spectrogram: Timber Wolf Call
[[file:img/timber-wolf.png]]
...
#+RESULTS: spectrogram: Mockingbird Call
[[file:img/mockingbird.png]]
...
* Exploring Coding Rate
#+begin_src R :results graphics file :file img/tau_vis.png
wpm <- 1:40
T <- (6/5) * wpm^-1

df = data.frame(wpm = wpm, T = T)

## plot(T ~ wpm)
## title("Fundamental unit tau vs WPM")

ggplot(df, aes(x = wpm, y = T)) +
  geom_point() +
  labs(title = "Tau versus WPM",
       x = "Words per Minute",
       y = "Tau") +
  theme_light()
#+end_src
