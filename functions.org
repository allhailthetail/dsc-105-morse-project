...
rms_amplitudes <- function(files) {
  ...
  for (file in files){
    ...
    waveform_n <- read_wav(file) %>%
      tuneR::normalize(pcm = FALSE, unit = "1", center = TRUE)
...
#+end_src
** Butterworth Filtering in bulk:
#+name: apply_bworth_filter
#+begin_src R
apply_bw_filter <- function(file, output_dir, freq_params = list(center = 610, bw = 70)) {
  name <- tools::file_path_sans_ext(basename(file))
  output_name <- paste0(output_dir, "/", name, "_bworth.wav")
  ...
  tuneR::writeWave(denoised_wave, output_name)
}
#+end_src

** Automatic Notch Filter:
Basically the same implementation as the Butterworth filter, but this includes
automatic frequency detection of the strongest portion of the signal (hopefully the CW)

#+name: apply_anf_filter
#+begin_src R
apply_anf_filter <- function(file, output_dir, bw = 70) {
  name <- tools::file_path_sans_ext(basename(file))
  ...
  print(
    cat('file:', name, '\n',
        'Calculated Center Freq:', center,
        'Range:', f_range)
  )

  denoised_wave <- cleaned_wave %>%
    bwfilter(from = f_range[1], to = f_range[2], output = "Wave") %>%
    tuneR::normalize(unit = "32")
