...
** Analysis of files with predictions:
#+begin_src R :file plots/general/density-wpm.png :results file graphics
basic_metadata %>%
  select(filename, duration, prediction, approx_wpm) %>%
  filter(prediction != "") %>%
  arrange(approx_wpm) -> files_have_prediction

median_wpm <- median(files_have_prediction$approx_wpm)
mean_wpm <- mean(files_have_prediction$approx_wpm)

files_have_prediction %>% ggplot(aes(x = approx_wpm)) +
  xlim(0,60) +
  geom_density(fill = "steelblue", alpha = 0.7, color = "darkblue") -> p

p %>% + geom_vline(xintercept = median_wpm, linetype = "dashed", color = "green") -> p1


p1 %>% + labs(
    title = "Density Plot of Approximate Words Per Minute (WPM)",
    subtitle = paste0("Median: ", round(median_wpm)),
    x = "Approximate WPM",
    y = "Density"
  ) +
  theme_minimal() -> p2
...
Their distribution os somewhat odd, since they measure things that otherwise shouldn't be related.
#+end_quote
...
** RMS Amplitude as a predictor:
It turned out that RMS Amplitude was useful to predict which files were free of noise and which weren't.

#+begin_src R :results file graphics :file plots/general/rmsa_boxplot.png
detailed_analysis$rms_amplitude %>%
  boxplot(main = "RMS Amplitude of Samples")

text(0.71, 0.4, labels = "Clean", col = "dark green", cex = 1.5)
text(0.8, 0.22, labels = "Severe Noise", col = "dark red", cex = 1.5)
#+end_src


** Cleanest Files with Predictors:
#+begin_src R
detailed_analysis %>% filter(rms_amplitude > .4, prediction != "") %>%
  select(filename, duration, zcr, prediction, rms_amplitude)
#+end_src

** Noisiest Files with Predictors:
...
"Low frequency noise might not be removed out properly."
#+end_quote
** Butterworth en masse:
Given the success of the butterworth filter, let's apply that to all the files:

First, let's tidy up that sub-folder:
#+begin_src sh :results silent
rm out/*.wav
#+end_src

#+begin_src R :results silent
params <- list(center = 610, bw = 70)

files <- basic_metadata$filename

# Loop and apply to all files:
for (file in files) {
  apply_bw_filter(file, output_dir = 'out/bworth', freq_params = params)
}
#+end_src

* Step 4: Creating an Automatic Notch Filter:
I then realized this was a good opportunity to automate this and create an Automatic Notch Filter, which the ARRL Handbook (Ch 12) indicates is one of the strengths of Digital Signal Processing. Surely this would greatly further future progress... So, let's get busy with that...

#+begin_src R :results silent
for (file in files) {
  apply_anf_filter(file, output_dir = 'out/anf', bw = 70)
}
#+end_src
...
output_dir <- 'out/anf'
output_name <- paste0(output_dir, "/", name, "_anf.wav")

cleaned_wave <- file %>%
  tuneR::readWave() %>%
  wav_remove_NAs()

cleaned_wave %>% seewave::cutw(from = 0.5, to = 3, output = 'Wave') %>%
  seewave::spec(plot = FALSE) -> spectrum

peak_index <- which.max(spectrum[, 2]) # Amplitude is column 2

## center <- spectrum[peak_index, 1]
center <- round(spectrum[peak_index, 1] * 1000)
...
* Updating metadata
Basically repeating the initial metadata gathering process, but for newly cleaned files:
#+begin_src R
"out/anf" -> audio_files_path
list.files(audio_files_path,
           full.names=TRUE) -> audio_files # this is a vec of all our audio files

print("--------")
print("Every file was processed?")
length(audio_files) == 200
print("--------")

# Add in predictions from other df:
anf_metadata <- wav_metadata_multi(audio_files)
anf_metadata$prediction <- basic_metadata$prediction
anf_metadata$approx_wpm<- basic_metadata$approx_wpm

print("--------")
print("rows match?")
all.equal(colnames(basic_metadata), colnames(anf_metadata))
print("--------")
# preview:
head(anf_metadata)

# Separate Detailed Analysis:
anf_detailed_analysis <- anf_metadata
#+end_src

** Generate updated waveforms:

#+begin_src R :results silent
# Generate plots for each:
audio_files %>% plot_waveforms('plots/waveforms/anf')
#+end_src

* Visualizing the filtering results:
** Updated RMSE vs ZCR:
#+begin_src R :results graphics file :file plots/general/anf-zcr-vs-rms.png
rms_amplitudes(anf_metadata$filename) -> anf_detailed_analysis$rms_amplitude

ggplot(anf_detailed_analysis, aes(zcr, rms_amplitude)) +
  labs(title = 'RMS Amplitude vs Zero-crossing Rate - Filtered Files', x = 'ZCR', y = 'RMS Amplitude') +
  geom_point(color = 'skyblue')
#+end_src

#+begin_quote
This is actually what we should expect...
A nice [[https://www.youtube.com/watch?v=EycaSbIRx-0][video]] that describes how ZCR and rms energy "correlate"
#+end_quote

* Putting together a unified dataset:
#+begin_src R

anf_subset <- anf_detailed_analysis %>%
  select(zcr, rms_amplitude)

detailed_analysis$zcr_anf <- anf_subset$zcr
detailed_analysis$rms_amplitude_anf <- anf_subset$rms_amplitude

df_final <- detailed_analysis %>%
  relocate(zcr_anf, .after = zcr) %>%
  relocate(approx_wpm, .after = duration) %>%
  relocate(rms_amplitude_anf, .after = rms_amplitude) %>%
  select(-minAmp, -medAmp, -maxAmp, -samples, -sampleRate, -stereo, -pcm, -bitdepth) %>%

  # Relocate 'prediction' to the very end of the final dataframe
  relocate(prediction, .after = last_col())

# 3. Write the final CSV file
write.csv(df_final, file = 'csv/analysis.csv', row.names = FALSE)
#+end_src
...
biber ./abstract
pdflatex ./abstract.tex # Biber requires running 2x
#+end_src
...
* Pre-commit Clean-up:
 ...
 #+name: Clean up Babel output blocks
 #+begin_example elisp
